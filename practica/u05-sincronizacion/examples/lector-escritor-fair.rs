...
struct ReadWrite {
    readers: i32,
    writing: bool,
    queue: u32, // Número de turno asignado a cada lector o escritor.
    next: u32, // Número de turno que se está atendiendo actualmente.
}
...
fn main() {
    ...
    // Guardamos una cola de turnos para los lectores y escritores, para garantizar que accedan al recurso de manera justa. Cada vez que un lector o escritor quiere acceder al recurso, se le asigna un número de turno (queue) y se espera a que sea su turno (comparando con next) para acceder al recurso. Esto evita que los lectores puedan bloquear indefinidamente a los escritores si siempre hay lectores queriendo acceder al recurso.
